...
const Router = {
    routes: [],
    scripts: [],
    config: {mode: null}, // {mode: "history" | "href"}
    ...
    renderPage: () => {
        ...
        if (Router.checkRoute() !== -1) {
            if (Router.routes[Router.checkRoute()].object.attributes.from !== undefined) {
                ...
                xhr.send();
            } else if (Router.scripts.map(val => val.route).includes(Router.routes[Router.checkRoute()].route)) Router.scripts.filter(val => val.route == Router.routes[Router.checkRoute()].route)[0].callback();
    ...
    goTo: route => {
        if (Router.config.mode == "history") history.pushState(null, null, window.location.origin + route); else if (Router.config.mode == "href") window.location.href = window.location.origin + route;
        Router.renderPage();
...
Router.makeLinks();
if (Router.config.mode == "history") Router.goTo(window.location.pathname); else if(Router.config.mode == "href") Router.renderPage(); // Render the current page by needed config
